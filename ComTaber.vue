    ...
    <div class="comTaber">
        <div v-if="state.search.length" class="container-header">
            ...
            >
                ...
                <a-form-item>
                    <a-button class="mr10" html-type="submit" type="primary">
        ...
        <div class="container-main">
            ...
            <a-table
                ...
                :loading="loading"
                :data="state.result"
                :pagination="state.hidePaging ? false : pagination"
                :bordered="{ cell: true }"
                :sticky-header="true"
                ...
                @selection-change="JSelectionChange"
            >
                <template #empty>
                    <div v-if="state.emptyTip" v-html="state.emptyTip"></div>
                    <a-empty v-else />
                </template>
                <template #columns>
                    <a-table-column
                    ...
                    <template v-if="state.operations.length">
                        ...
                        >
                            <template #cell="{ record, rowIndex }">
                                <template
                                    v-for="item in state.operations"
                                    :key="item.name"
                                >
                                    <a-link
                                        v-if="
...
const state = reactive({
    ...
    // 表格的行选择器配置
    rowSelection: cfg.rowSelection,
    // 表格行 key 的取值字段
    rowKey: cfg.rowKey,
    // 空数据提示
    emptyTip: cfg.emptyTip || '',
});
...
const query = async (queryParam) => {
    ...
    try {
        let params = Object.assign(state.filters, queryParam);

        if (cfg.beforeSearch) {
            params = cfg.beforeSearch(params);
        }

        const response = (await Utils.ajax(cfg.store, params)) || {};
        let resList = response.results;
        let resPaging = response.page;

        if (!resList && Array.isArray(response)) {
            resList = response || [];
            resPaging = { page: 1, limit: 20, total: resList.length || 0 };
        } else {
            resList = resList || [];
            resPaging = resPaging || {};
        }

        state.result = resList;
        state.paging = resPaging;
    } finally {
        loading.value = false;
    }

    const scrlTaberBd = document.querySelector('.arco-table-body');
    if (scrlTaberBd) {
        scrlTaberBd.scrollTo(0, 0);
    }
    window.scrollTo(0, 0);
};
const getCurPgData = () => {
    return Utils.deepCopy(state.result);
};
// 翻页
const JPaging = (e) => {
...
const JSelectionChange = (e) => {
    emits('onSelectionChange', e);
};
// 搜索
const JSearch = () => {
    state.filters.page = 1;
...
// 关联搜索
const JSearchChange = async (v, curLine) => {
    if (curLine?.refreshTarget) {
        const targetField = curLine.refreshTarget(v);
        const targetSearch = (state.search || []).find((sItem) => {
            return sItem?.dataIndex === targetField;
        });

        if (targetSearch && targetSearch.refreshOptions) {
            state.filters[targetSearch.dataIndex] = undefined;
            targetSearch.options = await targetSearch.refreshOptions();
        }
    }
};
const resetSelectKeys = () => {
    domTable.value.selectAll(false);
};
